			...
			//if($(win).width()>App.mobile){
				if($('.button_up').length){
					...
					$(win).scroll(function(){
						...
						footer_position=$('footer').position().top-window.innerHeight,
						button=$('.button_up');
						if(scrolltop>300){
							button.fadeIn();
						} else {
				...
				$(win).on('resize', function(){
					if($(win).width()>App.table){
						var win_height=window.innerHeight,
							...
							console.log('SLIDER HEIGHT '+win_height)
					} else {
		...
		initCatalog: function(){
			...
			if(catalog_container.length){
				...
				$(doc).on('click', '.share_button', function(e){
					e.preventDefault();
                    $('.dropdown_share').toggle();
					var t=$('.share_button');
					//dropdown=$('.dropdown_share');
                    if(t.hasClass('active')){
                        
						$('.dropdown_share').bind('clickoutside', function(){
							$('.dropdown_share').hide()
							t.removeClass('active')
							$('.dropdown_share').unbind('clickoutside');
						});
                    }
                    else{
						$('.dropdown_share').unbind('clickoutside');
